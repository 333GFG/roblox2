...
function DistanceESP:_createPlayerObjects(player)
    ...
    text.Size = self.Size
    text.Center = true           -- авто-центрирование по X
    text.Outline = true
    text.OutlineColor = Color3.new(0, 0, 0)
    -- НЕ указываем text.Font! Оставляем дефолт (UI), чтобы избежать
    -- конфликтов EnumItem в старых версиях эксплойтов
    text.Text = "0"
...
function DistanceESP:_updatePlayer(objs)
    ...
    -- Центрирование через Center = true, НЕ через TextBounds
